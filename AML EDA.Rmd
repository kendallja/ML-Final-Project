...
```{r}
# read in data
raw_data <- read.csv(file="Training_Data_Release", header=TRUE, sep=",")
```

```{r}
# add survivor column to set 
raw_data$STATUS <- ifelse(raw_data$Overall_Survival >= 260, 'SURVIVED', 
                                ifelse(raw_data$Overall_Survival < 260, 'NOT-SURVIVED', NA))
```
...
logistic_regression <- train(STATUS~ CCND3, data = LR_df, method = "glm", family= "binomial", trControl = ctrl)
```
...
plot(x = roc(predictor = logistic_regression$pred$benign, response = logistic_regression$pred$obs)$specificities, y = roc(predictor = logistic_regression$pred$benign, response = logistic_regression$pred$obs)$sensitivities, col= "purple", xlim = c(1, 0), type ="l", ylab = "Sensitivity", xlab = "Specificity")
legend("right", legend = paste("benign vs. malignant: ", roc(predictor = logistic_regression$pred$benign, response = logistic_regression$pred$obs)$auc
, sep = ""), col = c("purple"), fill = c("purple"))
```

Confusion Matrix for Test Data
```{r}
logistic_regression_predict_class <- predict(logistic_regression, newdata = test_classifier)
confusionMatrix(logistic_regression_predict_class, reference = test_classifier$Class)
```